...
import os
import numpy as np
from data_processor import convert_examples_to_features, convert_features_to_dataset, DataProcessor, LABEL2ID
from bert_for_cs import BertForCS
...
def main():
    ...
    test_dataset = convert_features_to_dataset(convert_examples_to_features(
        examples=test_eg, label2id=LABEL2ID, max_seq_length=30, tokenizer=tokenizer))

    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    config = config_class.from_pretrained(args.model_name_or_path, num_labels=2)
    model = BertForCS.from_pretrained(args.model_name_or_path, config=config, num_labels=2)
