...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x/np.reshape(np.linalg.norm(x, ord=2, axis=1), (-1, 1))
    ### END YOUR CODE
...
def skipgram(currentWord, C, contextWords, tokens, inputVectors, outputVectors, 
    ...
    for word in contextWords:
        target = tokens[word]
        tmp_cost, tmp_gradPred, tmp_grad = word2vecCostAndGradient(input_vector, target, outputVectors, dataset)
        cost += tmp_cost
        gradIn[index] += tmp_gradPred
        gradOut += tmp_grad
    # raise NotImplementedError
    ### END YOUR CODE
